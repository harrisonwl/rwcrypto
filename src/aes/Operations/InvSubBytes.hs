{-# LANGUAGE DataKinds #-}
module Aes.Operations.InvSubBytes(invsubbytes) where
...
invsubbytes s = generate $ \ i ->
                generate $ \ j -> let xij = (s `index` i) `index` j in
                                     invsbox xij

invsbox :: W 8 -> W 8
invsbox w = lkup (mkix w)
   where
     
     lkup :: (Index , Index) -> W 8
     lkup (i , j) = (invsboxTable `index` i) `index` j
...
isbc :: State -- InvSubBytes rst is:
isbc = mkstate [ [0x44, 0x3f, 0x5c, 0x1f]
               , [0x7e, 0xe1, 0x7f, 0x42]
               , [0x4b, 0xfb, 0x59, 0xaf]
               , [0x91, 0xc1, 0xdb, 0xb4]]
