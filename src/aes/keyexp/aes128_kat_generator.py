#!/usr/bin/env python3
"""
Known Answer Test (KAT) generator for AES-128 key expansion.
Generates random 128-bit keys, computes key expansion, and writes
Haskell KAT definitions to KATs128.hs.
"""

import sys
import secrets

from aes128_key_expansion import key_expansion


def key_bytes_to_integer(key: list[int]) -> int:
    """Convert 16-byte key (big-endian) to Integer."""
    n = 0
    for b in key:
        n = (n << 8) | b
    return n
...
def format_kat_for_haskell(key_int: int, w: list[list[int]]) -> str:
    ...
    """
    key_hex = f"0x{key_int:032x}"  # 128 bits = 32 hex digits
    w_pairs = ", ".join(
...
def write_kats_hs(
    kats: list[tuple[int, list[list[int]]]], path: str = "KATs128.hs"
) -> None:
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python aes128_kat_generator.py <number_of_KATs>", file=sys.stderr)
        sys.exit(1)
    try:
        n = int(sys.argv[1])
    except ValueError:
    ...
    write_kats_hs(kats)
    print(f"Wrote {n} KAT(s) to KATs128.hs", file=sys.stderr)
